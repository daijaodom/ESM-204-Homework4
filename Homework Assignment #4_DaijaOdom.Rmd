...
author: "Daija Odom and Devon Rossi and Logan Ossentjuk"
date: "5/16/2022"
output:
  html_document:
    toc: yes
...
cost_comparison <- cost_df %>% 
  ...
  mutate(cost_a = cost_policy_a/1e12) %>% 
  mutate(cost_b = cost_policy_b/1e12)
```

```{r}
ggplot(data = cost_comparison) +
 geom_line(aes(x = cost/1e12, y = cost_policy_a/1e12)) + 
  geom_line(aes(x = cost/1e12, y = cost_policy_b/1e12)) +
  theme_minimal() + 
  labs(x = "Policy Implementation Cost", y = "Total Expected Cost") +
  geom_text(aes(x = 4000, y = 3200, label = "A"), size = 4) +
  geom_text(aes(x = 4000, y = 5300, label = "B")) +
